...
The baseline agent is one model deployment plus `agent/instructions.md`. `scripts/2_deploy_agent.py` builds `PromptAgentDefinition(model="chat-model", instructions=...)` with no tools, and dev, test, and prod are three agents inside one Foundry project. The pipeline identity holds Foundry User on the resource group and nothing else.

Sandbox note: web search, file search, MCP and code execution all work in your sandbox as written below. For RAG with Azure AI Search, create the search service and its role assignments yourself (lesson 2.8b shows how, Basic tier, managed identity) and leave them out of the pipeline. The pipeline identity in your sandbox cannot hold Contributor or Role Based Access Control Administrator, so where a section says the pipeline creates a resource or a role assignment, you do that step once by hand instead.
