...
for k in AZURE_SUBSCRIPTION_ID AZURE_RESOURCE_GROUP GITHUB_REPO FOUNDRY_ACCOUNT FOUNDRY_PROJECT; do
  [[ -n "${!k:-}" && "${!k}" != *"<"* ]] || { echo "Edit .env (and run 0_prepare) first: $k is still a placeholder or empty."; exit 1; }
done
az account set --subscription "$AZURE_SUBSCRIPTION_ID"
sub="$AZURE_SUBSCRIPTION_ID"
tenant="$(az account show --query tenantId -o tsv)"
rg="$AZURE_RESOURCE_GROUP"; repo="$GITHUB_REPO"
...
MSYS_NO_PATHCONV=1 az role assignment create --assignee-object-id "$principal_id" --assignee-principal-type ServicePrincipal \
  --role "$foundry_user" --scope "/subscriptions/$sub/resourceGroups/$rg" --query id -o tsv --only-show-errors > /dev/null
echo "Foundry User on $rg"
...
gh variable set FOUNDRY_ACCOUNT --body "$FOUNDRY_ACCOUNT" --repo "$repo"
gh variable set FOUNDRY_PROJECT --body "$FOUNDRY_PROJECT" --repo "$repo"
gh variable set AGENT_NAME --body "$AGENT_NAME" --repo "$repo"
echo "Repository variables AZURE_RESOURCE_GROUP, FOUNDRY_ACCOUNT, FOUNDRY_PROJECT, AGENT_NAME set."
