...
if az cognitiveservices account deployment show --name "$FOUNDRY_ACCOUNT" --resource-group "$rg" --deployment-name chat-model --query name -o tsv > /dev/null 2>&1; then
  if out="$(az cognitiveservices account deployment delete --name "$FOUNDRY_ACCOUNT" --resource-group "$rg" --deployment-name chat-model 2>&1)"; then
    echo "chat-model deployment deleted"
  elif [[ "$out" == *ScopeLocked* ]]; then
    echo "[SKIP] chat-model stays: the Foundry account is locked, so deployments cannot be deleted by you. It costs nothing while idle; Frankie removes it."
  else
    echo "[WARN] chat-model could not be deleted: $out"
  fi
else
